---
description: SELLY — update to the latest version (git pull → re-run setup → restart daemon)
---

# /selly-upgrade — update SELLY

Pulls the latest SELLY, re-runs the idempotent installer (refreshes launchers + config, runs any
...
home = `cat ~/.selly/home` (fallback: the current SELLY checkout)
cd "$home"
...
after = `cat VERSION`
if before != after:
    say "Updated SELLY $before → $after."
...
- **No data risk.** Upgrade only touches code + launchers + `~/.selly`; per-deployment state in
  `data/` is never modified (migrations, when present, are explicit and reversible).
- **Auto-upgrade (optional):** if `selly-config get auto_upgrade` is `true`, a shared-repo
