---
description: SELLY — guided install & onboarding (Stage 2; the bootstrap hands off here)
---

# /selly-install — guided onboarding (Stage 2)

The conversational onboarding the bootstrap installer hands off to. The user never types
`/selly-install` on a fresh machine — they install by cloning the repo and running `./setup`:
...
`follow .claude/commands/selly-install.md` — passing the verified choice as `$SELLY_HARNESS`. So
by the time this runs, the file is on disk and the harness is already chosen + authenticated (§0
trusts it). After setup it's also a normal slash command for re-runs; `/selly` handles piecemeal
changes.

SELLY is **OS-agnostic and harness-agnostic**: every OS-specific step goes through `bin/platforms/`
...
`skills/selly-config.md`, `skills/channel/onboarding.md`.

## Flow

```
say  intro  (reuse skills/channel/intro.md body — "I'm SELLY …")
...
#    and exports it as $SELLY_HARNESS — so trust it and DON'T re-ask.
if  env $SELLY_HARNESS is set:
     run  python3 bin/install.py harness --name "$SELLY_HARNESS"   # confirm still signed in (exit 0)
     -> exit 0 -> say "Using <harness>." ; <harness> = $SELLY_HARNESS.
...
# 1. TARGET — how should SELLY run?  (SELLY is designed to run ALWAYS-ON.)
ask  "How should SELLY run?"
...
# 8. AUTONOMY — the step that makes SELLY run by itself (skills/selly-config.md)
...
ask  "Want me to BUY for you as well (search marketplaces + negotiate on your behalf)?"
  ...
  skip -> say "You can enable buying any time via /selly -> buying."

# 8c. GLOBAL LAUNCHERS — make /selly, /sell, /buy work from any project (gstack-style)
...
run  python3 bin/selly-config set home "$(pwd)"                # record runtime dir (~/.selly/home)
...
run  python3 bin/install.py supervisor --no-dry-run   # platform module: launchd plists (macOS) or
     ...
     -> interactive target: skip; tell the user to keep a `/selly-run` (or `/loop /selly-run`) session
...
say  "✅ SELLY is set up. Send /list with photos to sell, /search to buy, or open the /selly menu
...
- The autonomy step (§8) is what lets SELLY list, search, and check chats unattended — it wires the
  business-approval preset AND the harness allow-list/approval-mode together (see
  `skills/selly-config.md` → "Two layers of autonomy").
...
- Re-running is safe and idempotent; for piecemeal changes after setup, use `/selly`.
